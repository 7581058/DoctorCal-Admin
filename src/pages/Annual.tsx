...
import { ANNUAL_COLUMN } from '@/constants/annual';
...
const Annual = () => {
  ...
  const getAnnualList = async (page: number, selectedSort: string) => {
    try {
      setIsLoading(true);
      const res = await annual({ page: page, sort: `createdAt,${selectedSort}` });
      setRequests(res.item);
    } catch (error) {
      setAlert({
        isOpen: true,
        content: `연차 결재 리스트 조회 실패\n${error}`,
        type: 'error',
      });
    } finally {
      setIsLoading(false);
  ...
  return (
    <Container>
      ...
      {isLoading && <Loading />}
      <TitleContainer>
        <PageTitle>{PAGE_TITLE_TEXTS.annualTitle}</PageTitle>
        <SubTitle>
          <CustomSelect options={SORT_SELECT_OPTION} value={value} onChange={handleValueChange} />
        </SubTitle>
      </TitleContainer>
      <GridTable rowData={requests} columnsData={ANNUAL_COLUMN} />
    </Container>
  );
};
